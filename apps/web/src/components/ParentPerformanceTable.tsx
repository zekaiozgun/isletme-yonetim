...
type SortKey = 'avg_daily_gain_kg' | 'avg_profit_try' | 'loss_rate';

const SORT_COLUMNS: { key: SortKey; label: string }[] = [
  { key: 'avg_daily_gain_kg', label: 'Ort. Günlük Kilo Artışı' },
  { key: 'avg_profit_try', label: 'Ort. Kâr/Zarar' },
...
function formatProfit(value: unknown): string {
 ...
 * sütun başlığına tıklayınca aktif sıralama kriteri değişir (kilo
 * artışı/kâr-zarar için büyükten küçüğe, kayıp oranı için küçükten
 * büyüğe - "en iyi" her zaman üstte). Ebeveyn kimliği (Anne küpe no ya
 * da Baba gösterim önceliği) çağıran taraftan `getParentLabel` ile
 * gelir, bu bileşen ebeveyn türünden bağımsızdır. */
export function ParentPerformanceTable({
  ...
  const csvHeaders = [parentColumnLabel, 'Yavru Sayısı', 'Ort. Günlük Kilo Artışı', 'Ort. Kâr/Zarar (TL)', 'Kayıp Oranı'];
  const csvRows = sortedRows.map((row) => [
    getParentLabel(row),
    offspringCountLabel(row),
    formatGain(row.avg_daily_gain_kg),
    formatProfit(row.avg_profit_try),
  ...
  const pdfColumns = [
    ...
    { label: 'Ort. Kâr/Zarar (TL)', width: 'narrow' as const },
            ...
            {sortedRows.map((row, index) => {
              ...
              return (
                <tr key={index} data-search={label.toLocaleLowerCase('tr-TR')} className="divide-x divide-slate-100">
                  ...
                  <td className="px-[0.5ch] py-1.5 text-slate-700">{formatProfit(row.avg_profit_try)}</td>
